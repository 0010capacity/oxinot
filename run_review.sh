#!/bin/bash

# ==============================================================================
# AI Code Review System (oxinot) - Improved Version (Bash 3.2 Compatible)
# ==============================================================================

# 1. 설정
GEMINI_CMD="gemini"
BASE_OUTPUT_DIR="docs/code_reviews"
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
OUTPUT_DIR="${BASE_OUTPUT_DIR}/${TIMESTAMP}"
mkdir -p "$OUTPUT_DIR"

# 컨텍스트 크기 제한 (대략 문자 수, Gemini 2.0 Flash 기준 약 1M 토큰)
MAX_CONTEXT_CHARS=100000

# API Rate Limit 방지: 동시 실행 수 제한
MAX_PARALLEL=3

# 상태 아이콘
ICON_WAIT="⏳"
ICON_DONE="✅"
ICON_FAIL="❌"
ICON_CHUNK="📦"

# 2. 역할 및 세부 프롬프트 정의
ROLES=(
    "Frontend_UI_UX"
    "Frontend_Editor_Outliner"
    "Frontend_State_Logic"
    "Backend_Rust_Core"
    "Backend_DB_Commands"
    "Security_Infra"
)

# 역할별 타겟 디렉토리 및 확장자 정의
# Bash 3.2 compatibility: Use functions to simulate associative arrays
get_target() {
    case "$1" in
        "Frontend_UI_UX") echo "src/components src/styles src/theme" ;;
        "Frontend_Editor_Outliner") echo "src/outliner src/editor src/markdown" ;;
        "Frontend_State_Logic") echo "src/stores src/hooks src/contexts" ;;
        "Backend_Rust_Core") echo "src-tauri/src/services src-tauri/src/main.rs src-tauri/src/lib.rs" ;;
        "Backend_DB_Commands") echo "src-tauri/src/db src-tauri/src/commands src-tauri/src/models" ;;
        "Security_Infra") echo "src-tauri/tauri.conf.json src-tauri/capabilities src/tauri-api.ts" ;;
    esac
}

get_ext() {
    case "$1" in
        "Frontend_UI_UX") echo "tsx css ts" ;;
        "Frontend_Editor_Outliner") echo "tsx ts css" ;;
        "Frontend_State_Logic") echo "ts tsx" ;;
        "Backend_Rust_Core") echo "rs" ;;
        "Backend_DB_Commands") echo "rs" ;;
        "Security_Infra") echo "json ts" ;;
    esac
}

get_prompt() {
    case "$1" in
        "Frontend_UI_UX")
            cat <<'EOF'
당신은 'UI/UX & Design System Specialist'입니다.
다음 코드를 분석하고 한국어로 리뷰 리포트를 작성해주세요.

[출력 형식]
## 🎨 UI/UX 코드 리뷰

### ⚠️ 심각도 높음 (High Priority)
런타임 오류, UI 깨짐, 접근성 위반 등 즉시 수정이 필요한 문제만 이곳에 작성하세요.
문제가 없다면 이 섹션을 생략하세요.

형식: [파일명:라인] 문제 설명 → 해결 방법

### ⚡ 심각도 중간 (Medium Priority)
성능 저하, 일관성 부족, 유지보수성 저하 등 개선이 권장되는 문제를 작성하세요.
문제가 없다면 이 섹션을 생략하세요.

형식: [파일명:라인] 문제 설명 → 해결 방법

### 💡 기존 코드 개선 제안 (Code Improvements)
현재 코드의 리팩토링, 최적화, 구조 개선 등을 제안하세요.
각 제안에는 Before/After 코드 예시를 포함해주세요.

### 🚀 새로운 기능 제안 (Feature Suggestions)
현재 프로젝트 구조를 분석한 결과, 실제로 사용자에게 가치가 있을 것 같은 새로운 기능을 제안하세요.
각 제안에는 다음을 포함하세요:
- 기능 설명
- 구현 난이도 (쉬움/보통/어려움)
- 예상 효과 (사용자 경험 개선, 생산성 향상 등)

**제안이 없다면 해당 섹션을 생략하거나 "현재 구조가 적절합니다"라고 명시하세요.**

[리뷰 체크리스트]
다음 항목들을 확인하되, 문제가 있는 것만 리포트에 포함하세요:
□ UI 구조 안정성: 레이아웃이 깨질 수 있는 결함 (flex/grid 오용, 고정 크기 등)
□ 스타일링 일관성: Magic Numbers 대신 테마 변수 사용 여부
□ 반응형 및 엣지 케이스: 다양한 화면 크기, 긴 텍스트 처리
□ 접근성: aria-label, 포커스링, 키보드 네비게이션
□ 성능: 불필요한 리렌더링, 비효율적인 CSS

[중요 지침]
- 구체적인 파일명과 라인 번호를 반드시 명시하세요
- 확신이 없는 경우 '검토 필요' 또는 '확인 권장'으로 표시하세요
- 여러 청크가 있는 경우, 이전 청크에서 지적한 것과 동일한 패턴은 '앞서 언급한 패턴과 동일'로 간략히 처리하세요
EOF
            ;;
        "Frontend_Editor_Outliner")
            cat <<'EOF'
당신은 'Editor Engine Engineer'입니다.
다음 코드를 분석하고 한국어로 리뷰 리포트를 작성해주세요.

[출력 형식]
## ✍️ 에디터 엔진 코드 리뷰

### ⚠️ 심각도 높음 (High Priority)
데이터 손실, 크래시, 심각한 성능 저하 등 즉시 수정이 필요한 문제만 이곳에 작성하세요.
문제가 없다면 이 섹션을 생략하세요.

형식: [파일명:라인] 문제 설명 → 해결 방법

### ⚡ 심각도 중간 (Medium Priority)
메모리 누수, 동기화 이슈, 사용성 저하 등 개선이 권장되는 문제를 작성하세요.
문제가 없다면 이 섹션을 생략하세요.

형식: [파일명:라인] 문제 설명 → 해결 방법

### 💡 기존 코드 개선 제안 (Code Improvements)
현재 에디터 코드의 리팩토링, 최적화, 구조 개선 등을 제안하세요.
각 제안에는 Before/After 코드 예시를 포함해주세요.

### 🚀 새로운 기능 제안 (Feature Suggestions)
현재 에디터 구조를 분석한 결과, 실제로 사용자에게 가치가 있을 것 같은 새로운 기능을 제안하세요.
각 제안에는 다음을 포함하세요:
- 기능 설명
- 구현 난이도 (쉬움/보통/어려움)
- 예상 효과 (편집 효율, 안정성 등)

**제안이 없다면 해당 섹션을 생략하거나 "현재 구조가 적절합니다"라고 명시하세요.**

[리뷰 체크리스트]
다음 항목들을 확인하되, 문제가 있는 것만 리포트에 포함하세요:
□ 성능 최적화: 대량 블록 처리 시 가상화, 렌더링 병목
□ 상태 동기화: CodeMirror와 React 상태 간 불일치 가능성
□ 안정성: IME 입력(한글), 드래그 앤 드롭 등 복잡한 동작의 상태 관리
□ 메모리 관리: 이벤트 리스너 해제 누락, 메모리 누수
□ 에디터 UX: 커서 이동, 선택, 편집 흐름의 자연스러움

[중요 지침]
- 구체적인 파일명과 라인 번호를 반드시 명시하세요
- 확신이 없는 경우 '검토 필요' 또는 '확인 권장'으로 표시하세요
- 여러 청크가 있는 경우, 이전 청크에서 지적한 것과 동일한 패턴은 '앞서 언급한 패턴과 동일'로 간략히 처리하세요
EOF
            ;;
        "Frontend_State_Logic")
            cat <<'EOF'
당신은 'Frontend Architect'입니다.
다음 코드를 분석하고 한국어로 리뷰 리포트를 작성해주세요.

[출력 형식]
## 🔄 상태 관리 및 로직 리뷰

### ⚠️ 심각도 높음 (High Priority)
데이터 불일치, 경쟁 상태, 심각한 성능 저하 등 즉시 수정이 필요한 문제만 이곳에 작성하세요.
문제가 없다면 이 섹션을 생략하세요.

형식: [파일명:라인] 문제 설명 → 해결 방법

### ⚡ 심각도 중간 (Medium Priority)
불필요한 리렌더링, 복잡한 의존성, 개선 가능한 구조 등 개선이 권장되는 문제를 작성하세요.
문제가 없다면 이 섹션을 생략하세요.

형식: [파일명:라인] 문제 설명 → 해결 방법

### 💡 기존 코드 개선 제안 (Code Improvements)
현재 상태 관리 코드의 리팩토링, 최적화, 구조 개선 등을 제안하세요.
각 제안에는 Before/After 코드 예시를 포함해주세요.

### 🚀 새로운 기능 제안 (Feature Suggestions)
현재 아키텍처를 분석한 결과, 개발 경험이나 코드 품질을 향상시킬 수 있는 기능을 제안하세요.
각 제안에는 다음을 포함하세요:
- 기능 설명
- 구현 난이도 (쉬움/보통/어려움)
- 예상 효과 (개발자 경험, 유지보수성 등)

**제안이 없다면 해당 섹션을 생략하거나 "현재 구조가 적절합니다"라고 명시하세요.**

[리뷰 체크리스트]
다음 항목들을 확인하되, 문제가 있는 것만 리포트에 포함하세요:
□ 상태 관리 최적화: Selector 사용, 불필요한 리렌더링 방지
□ 커스텀 훅: 비즈니스 로직 분리, 메모이제이션(useMemo, useCallback)
□ 데이터 무결성: Race Condition, 에러 핸들링
□ 구조 문제: Zombie Child, 순환 참조
□ 타입 안전성: TypeScript 활용도

[중요 지침]
- 구체적인 파일명과 라인 번호를 반드시 명시하세요
- 확신이 없는 경우 '검토 필요' 또는 '확인 권장'으로 표시하세요
- 여러 청크가 있는 경우, 이전 청크에서 지적한 것과 동일한 패턴은 '앞서 언급한 패턴과 동일'로 간략히 처리하세요
EOF
            ;;
        "Backend_Rust_Core")
            cat <<'EOF'
당신은 'Rust Systems Programmer'입니다.
다음 코드를 분석하고 한국어로 리뷰 리포트를 작성해주세요.

[출력 형식]
## 🦀 Rust 코어 시스템 리뷰

### ⚠️ 심각도 높음 (High Priority)
패닉 가능성, 데드락, 데이터 경쟁 등 즉시 수정이 필요한 문제만 이곳에 작성하세요.
문제가 없다면 이 섹션을 생략하세요.

형식: [파일명:라인] 문제 설명 → 해결 방법

### ⚡ 심각도 중간 (Medium Priority)
비효율적인 메모리 사용, blocking I/O, 개선 가능한 패턴 등 개선이 권장되는 문제를 작성하세요.
문제가 없다면 이 섹션을 생략하세요.

형식: [파일명:라인] 문제 설명 → 해결 방법

### 💡 기존 코드 개선 제안 (Code Improvements)
현재 Rust 코드의 리팩토링, 최적화, idiomatic 패턴 적용 등을 제안하세요.
각 제안에는 Before/After 코드 예시를 포함해주세요.

### 🚀 새로운 기능 제안 (Feature Suggestions)
현재 백엔드 구조를 분석한 결과, 시스템 안정성이나 운영 효율성을 높일 수 있는 기능을 제안하세요.
각 제안에는 다음을 포함하세요:
- 기능 설명
- 구현 난이도 (쉬움/보통/어려움)
- 예상 효과 (안정성, 성능 등)

**제안이 없다면 해당 섹션을 생략하거나 "현재 구조가 적절합니다"라고 명시하세요.**

[리뷰 체크리스트]
다음 항목들을 확인하되, 문제가 있는 것만 리포트에 포함하세요:
□ 안전성: unwrap()/expect() 남발로 인한 런타임 패닉 위험
□ 성능: async 함수 내 blocking I/O, 불필요한 clone()
□ 동시성: Mutex/RwLock 사용 시 데드락 가능성
□ Rust 관용구: idiomatic Rust 패턴 사용 여부
□ 에러 처리: Result/Option의 적절한 사용

[중요 지침]
- 구체적인 파일명과 라인 번호를 반드시 명시하세요
- 확신이 없는 경우 '검토 필요' 또는 '확인 권장'으로 표시하세요
- 여러 청크가 있는 경우, 이전 청크에서 지적한 것과 동일한 패턴은 '앞서 언급한 패턴과 동일'로 간략히 처리하세요
EOF
            ;;
        "Backend_DB_Commands")
            cat <<'EOF'
당신은 'Database & API Designer'입니다.
다음 코드를 분석하고 한국어로 리뷰 리포트를 작성해주세요.

[출력 형식]
## 💾 데이터베이스 & API 리뷰

### ⚠️ 심각도 높음 (High Priority)
데이터 손실 위험, 보안 취약점, 심각한 성능 문제 등 즉시 수정이 필요한 문제만 이곳에 작성하세요.
문제가 없다면 이 섹션을 생략하세요.

형식: [파일명:라인] 문제 설명 → 해결 방법

### ⚡ 심각도 중간 (Medium Priority)
쿼리 비효율, 입력값 검증 부족, 확장성 제한 등 개선이 권장되는 문제를 작성하세요.
문제가 없다면 이 섹션을 생략하세요.

형식: [파일명:라인] 문제 설명 → 해결 방법

### 💡 기존 코드 개선 제안 (Code Improvements)
현재 DB/API 코드의 리팩토링, 최적화, 구조 개선 등을 제안하세요.
각 제안에는 Before/After 코드 예시를 포함해주세요.

### 🚀 새로운 기능 제안 (Feature Suggestions)
현재 데이터베이스와 API 구조를 분석한 결과, 시스템을 발전시킬 수 있는 기능을 제안하세요.
각 제안에는 다음을 포함하세요:
- 기능 설명
- 구현 난이도 (쉬움/보통/어려움)
- 예상 효과 (사용자 가치, 성능 등)

**제안이 없다면 해당 섹션을 생략하거나 "현재 구조가 적절합니다"라고 명시하세요.**

[리뷰 체크리스트]
다음 항목들을 확인하되, 문제가 있는 것만 리포트에 포함하세요:
□ 쿼리 최적화: N+1 문제, 인덱스 누락, 비효율적인 조회
□ 트랜잭션: 원자성이 필요한 작업의 트랜잭션 처리
□ IPC 인터페이스: Tauri Command의 입력값 검증, 타입 안전성
□ 확장성: 스키마 변경 용이성, 마이그레이션 전략
□ 데이터 무결성: 제약 조건, 외래 키, 일관성

[중요 지침]
- 구체적인 파일명과 라인 번호를 반드시 명시하세요
- 확신이 없는 경우 '검토 필요' 또는 '확인 권장'으로 표시하세요
- 여러 청크가 있는 경우, 이전 청크에서 지적한 것과 동일한 패턴은 '앞서 언급한 패턴과 동일'로 간략히 처리하세요
EOF
            ;;
        "Security_Infra")
            cat <<'EOF'
당신은 'Security & Infrastructure Engineer'입니다.
다음 코드를 분석하고 한국어로 리뷰 리포트를 작성해주세요.

[출력 형식]
## 🔒 보안 및 인프라 리뷰

### ⚠️ 심각도 높음 (High Priority)
보안 취약점, 과도한 권한, 민감 정보 노출 등 즉시 수정이 필요한 문제만 이곳에 작성하세요.
문제가 없다면 이 섹션을 생략하세요.

형식: [파일명:라인] 문제 설명 → 해결 방법

### ⚡ 심각도 중간 (Medium Priority)
설정 최적화 필요, 모범 사례 미적용 등 개선이 권장되는 문제를 작성하세요.
문제가 없다면 이 섹션을 생략하세요.

형식: [파일명:라인] 문제 설명 → 해결 방법

### 💡 기존 설정 개선 제안 (Configuration Improvements)
현재 보안 설정 및 인프라 구성의 개선 방안을 제안하세요.
각 제안에는 Before/After 설정 예시를 포함해주세요.

### 🚀 새로운 기능 제안 (Feature Suggestions)
현재 보안 설정과 인프라 구조를 분석한 결과, 보안을 강화하거나 운영을 개선할 수 있는 기능을 제안하세요.
각 제안에는 다음을 포함하세요:
- 기능 설명
- 구현 난이도 (쉬움/보통/어려움)
- 예상 효과 (보안 강화, 운영 효율 등)

**제안이 없다면 해당 섹션을 생략하거나 "현재 설정이 적절합니다"라고 명시하세요.**

[리뷰 체크리스트]
다음 항목들을 확인하되, 문제가 있는 것만 리포트에 포함하세요:
□ 보안 설정: Tauri capabilities 최소 권한 원칙 준수
□ 입력값 검증: XSS, 인젝션 등 취약점 방어
□ 설정 최적화: tauri.conf.json 및 빌드 설정의 보안성
□ 민감 정보: 하드코딩된 비밀 키, 경로, API 키
□ 권한 관리: 파일 시스템, 네트워크 접근 권한

[중요 지침]
- 구체적인 파일명과 라인 번호를 반드시 명시하세요
- 확신이 없는 경우 '검토 필요' 또는 '확인 권장'으로 표시하세요
- 여러 청크가 있는 경우, 이전 청크에서 지적한 것과 동일한 패턴은 '앞서 언급한 패턴과 동일'로 간략히 처리하세요
EOF
            ;;
    esac
}

# 3. 유틸리티 함수

# Gemini CLI 존재 확인
check_gemini_cli() {
    if ! command -v "$GEMINI_CMD" &> /dev/null; then
        echo "❌ 오류: Gemini CLI를 찾을 수 없습니다."
        echo "   설치: npm install -g @google/generative-ai"
        exit 1
    fi
}

# 파일 수집 함수
collect_files() {
    local role=$1
    local targets=$(get_target "$role")
    local exts=$(get_ext "$role")
    local files=()

    for dir in $targets; do
        if [ ! -d "$dir" ] && [ ! -f "$dir" ]; then
            continue
        fi

        for ext in $exts; do
            # find -print0 is compatible with macOS
            while IFS= read -r -d '' file; do
                # Security_Infra 특수 케이스
                if [ "$role" == "Security_Infra" ]; then
                    if [[ "$file" != *"tauri.conf.json"* && \
                          "$file" != *"tauri-api.ts"* && \
                          "$file" != *"capabilities"* ]]; then
                        continue
                    fi
                fi
                files+=("$file")
            done < <(find "$dir" -name "*.$ext" -type f \
                -not -path "*/node_modules/*" \
                -not -path "*/dist/*" \
                -not -path "*/target/*" \
                -not -path "*/.git/*" \
                -print0 2>/dev/null)
        done
    done

    printf '%s\n' "${files[@]}"
}

# 파일을 청크로 분할
create_chunks() {
    local role=$1
    shift
    local files_in=("$@")
    local chunks=()
    local current_chunk=()
    local current_size=0

    for file in "${files_in[@]}"; do
        if [ ! -f "$file" ]; then
            continue
        fi

        local file_size=$(stat -f%z "$file" 2>/dev/null || stat -c%s "$file" 2>/dev/null)

        # 단일 파일이 제한을 초과하는 경우
        if [ "$file_size" -gt "$MAX_CONTEXT_CHARS" ]; then
            echo "⚠️  경고: $file 파일이 너무 큽니다 ($(($file_size / 1024))KB). 건너뜁니다." >&2
            continue
        fi

        # 현재 청크에 추가했을 때 제한 초과 여부 확인
        if [ $((current_size + file_size)) -gt "$MAX_CONTEXT_CHARS" ] && [ ${#current_chunk[@]} -gt 0 ]; then
            # 현재 청크 저장
            local IFS_BACKUP=$IFS
            IFS=,
            chunks+=("${current_chunk[*]}")
            IFS=$IFS_BACKUP
            current_chunk=()
            current_size=0
        fi

        current_chunk+=("$file")
        current_size=$((current_size + file_size))
    done

    # 마지막 청크 저장
    if [ ${#current_chunk[@]} -gt 0 ]; then
        local IFS_BACKUP=$IFS
        IFS=,
        chunks+=("${current_chunk[*]}")
        IFS=$IFS_BACKUP
    fi

    printf '%s\n' "${chunks[@]}"
}

# 프로젝트 구조 생성 (한 번만)
generate_project_structure() {
    if [ -f "$OUTPUT_DIR/project_structure.txt" ]; then
        cat "$OUTPUT_DIR/project_structure.txt"
        return
    fi

    local structure=""

    # tree 명령어가 있으면 사용
    if command -v tree &> /dev/null; then
        structure=$(tree -L 3 -I 'node_modules|dist|target|.git' --charset ascii)
    else
        # tree 없으면 find로 대체
        structure=$(find . -maxdepth 3 -type d \
            -not -path "*/node_modules/*" \
            -not -path "*/dist/*" \
            -not -path "*/target/*" \
            -not -path "*/.git/*" \
            2>/dev/null | sort)
    fi

    echo "$structure" > "$OUTPUT_DIR/project_structure.txt"
    echo "$structure"
}

# 컨텍스트 생성
build_context() {
    local chunk_files_str=$1
    local role=$2
    local chunk_num=$3
    local total_chunks=$4
    local context=""

    # 프로젝트 구조 추가 (첫 번째 청크에만)
    if [ "$chunk_num" -eq 0 ]; then
        context="$context"$'\n'"[프로젝트 전체 구조]"$'\n'
        context="$context"$(generate_project_structure)$'\n\n'
    fi

    # 청크 정보
    context="$context""[현재 청크 정보]"$'\n'
    context="$context""청크 $((chunk_num + 1))/$total_chunks"$'\n\n'

    # 파일 내용
    context="$context""[소스 코드]"$'\n'
    IFS=',' read -ra files_to_read <<< "$chunk_files_str"
    for file in "${files_to_read[@]}"; do
        if [ ! -f "$file" ]; then
            continue
        fi

        context="$context"$'\n'"--- FILE START: $file ---"$'\n'
        context="$context"$(cat "$file" 2>/dev/null || echo "# 파일 읽기 실패")$'\n'
        context="$context""--- FILE END: $file ---"$'\n'
    done

    echo "$context"
}

# 4. 병렬 실행 관리
# Bash 3.2 compatibility: Use dynamic variables for status and PIDs
set_status() {
    local role_safe=$(echo "$1" | tr ' ' '_')
    printf -v "STATUS_${role_safe}" "%s" "$2"
}

get_status() {
    local role_safe=$(echo "$1" | tr ' ' '_')
    eval "echo \"\$STATUS_${role_safe}\""
}

set_pid() {
    local key_safe=$(echo "$1" | tr ' |' '__')
    printf -v "PID_${key_safe}" "%s" "$2"
}

get_pid() {
    local key_safe=$(echo "$1" | tr ' |' '__')
    eval "echo \"\$PID_${key_safe}\""
}

unset_pid() {
    local key_safe=$(echo "$1" | tr ' |' '__')
    eval "unset PID_${key_safe}"
}

running_jobs=0
ACTIVE_KEYS=()

# 초기 상태 설정
for role in "${ROLES[@]}"; do
    set_status "$role" "$ICON_WAIT 대기 중..."
done

# 화면 출력 함수
draw_status() {
    tput cuu $((${#ROLES[@]} + 1))
    echo "🔄 진행 상황 (실행 중: $running_jobs/$MAX_PARALLEL)"
    for role in "${ROLES[@]}"; do
        tput el
        printf "  %-30s : %s\n" "$role" "$(get_status "$role")"
    done
}

# 화면 초기화
tput civis
clear

echo "🚀 [Start] AI 코드 리뷰를 시작합니다..."
echo "📅 실행 시간: $(date '+%Y-%m-%d %H:%M:%S')"
echo "📂 저장 경로: $OUTPUT_DIR"
echo ""

# Gemini CLI 확인
check_gemini_cli

# 5. 작업 실행
echo "🔍 파일 수집 중..."
# Bash 3.2 compatibility: Store files and chunks in dynamic variables
for role in "${ROLES[@]}"; do
    # Collect files
    role_files=()
    while IFS= read -r line; do
        role_files+=("$line")
    done < <(collect_files "$role")

    if [ ${#role_files[@]} -eq 0 ]; then
        set_status "$role" "⚠️  파일 없음"
        continue
    fi

    # Create chunks
    role_chunks=()
    while IFS= read -r line; do
        role_chunks+=("$line")
    done < <(create_chunks "$role" "${role_files[@]}")

    # Store in dynamic vars
    role_safe=$(echo "$role" | tr ' ' '_')
    printf -v "ALL_CHUNKS_${role_safe}_COUNT" "%s" "${#role_chunks[@]}"
    for i in "${!role_chunks[@]}"; do
        printf -v "ALL_CHUNKS_${role_safe}_${i}" "%s" "${role_chunks[$i]}"
    done

    echo "  $role: ${#role_files[@]}개 파일, ${#role_chunks[@]}개 청크"
done

echo ""
echo "📝 리뷰 작성 시작..."
echo ""

# 최초 공간 확보
for role in "${ROLES[@]}"; do echo ""; done

# 모든 작업 큐
job_queue=()
for role in "${ROLES[@]}"; do
    role_safe=$(echo "$role" | tr ' ' '_')
    count_var="ALL_CHUNKS_${role_safe}_COUNT"
    count=${!count_var:-0}

    for ((i=0; i<count; i++)); do
        chunk_var="ALL_CHUNKS_${role_safe}_${i}"
        job_queue+=("$role|$i|${!chunk_var}")
    done
done

# 작업 처리
job_index=0
while [ $job_index -lt ${#job_queue[@]} ] || [ $running_jobs -gt 0 ]; do
    # 새 작업 시작
    while [ $running_jobs -lt $MAX_PARALLEL ] && [ $job_index -lt ${#job_queue[@]} ]; do
        job="${job_queue[$job_index]}"
        role=$(echo "$job" | cut -d'|' -f1)
        chunk_num=$(echo "$job" | cut -d'|' -f2)
        chunk_files_str=$(echo "$job" | cut -d'|' -f3)

        role_safe=$(echo "$role" | tr ' ' '_')
        total_chunks_var="ALL_CHUNKS_${role_safe}_COUNT"
        total_chunks=${!total_chunks_var}

        if [ "$total_chunks" -gt 1 ]; then
            filename="${OUTPUT_DIR}/${role}_part$((chunk_num + 1))of${total_chunks}.md"
            set_status "$role" "$ICON_CHUNK 청크 $((chunk_num + 1))/$total_chunks 작성 중..."
        else
            filename="${OUTPUT_DIR}/${role}.md"
            set_status "$role" "$ICON_WAIT 작성 중..."
        fi

        (
            context=$(build_context "$chunk_files_str" "$role" "$chunk_num" "$total_chunks")
            prompt=$(get_prompt "$role")

            # Execute Gemini CLI
            final_prompt="$prompt"$'\n\n'"=== SOURCE CODE CONTEXT ===$context"
            $GEMINI_CMD "$final_prompt" > "$filename" 2>&1
            exit_code=$?

            # 청크 정보 파일 저장
            if [ $exit_code -eq 0 ]; then
                {
                    echo ""
                    echo "# 청크 정보"
                    echo "청크 번호: $((chunk_num + 1))/$total_chunks"
                    echo "파일 목록:"
                    IFS=',' read -ra cf <<< "$chunk_files_str"
                    for f in "${cf[@]}"; do echo "- $f"; done
                } >> "$filename"
            fi

            exit $exit_code
        ) &

        key="${role}_${chunk_num}"
        set_pid "$key" "$!"
        ACTIVE_KEYS+=("$key")
        running_jobs=$((running_jobs + 1))
        job_index=$((job_index + 1))
        draw_status
    done

    # 완료된 작업 확인
    NEW_ACTIVE_KEYS=()
    for key in "${ACTIVE_KEYS[@]}"; do
        pid=$(get_pid "$key")
        if ! kill -0 "$pid" 2>/dev/null; then
            wait "$pid"
            exit_code=$?

            role=$(echo "$key" | cut -d'_' -f1)
            chunk_num=$(echo "$key" | cut -d'_' -f2)

            role_safe=$(echo "$role" | tr ' ' '_')
            total_chunks_var="ALL_CHUNKS_${role_safe}_COUNT"
            total_chunks=${!total_chunks_var}

            if [ $exit_code -eq 0 ]; then
                if [ "$total_chunks" -gt 1 ]; then
                    # 모든 청크 완료 확인은 실제로는 모든 작업 루프가 끝난 뒤 처리하거나
                    # 마지막 청크가 끝났을 때를 기준으로 대략적으로 표시
                    if [ $((chunk_num + 1)) -eq "$total_chunks" ]; then
                        set_status "$role" "$ICON_DONE 완료 (${total_chunks}개 청크)"
                    fi
                else
                    set_status "$role" "$ICON_DONE 완료"
                fi
            else
                set_status "$role" "$ICON_FAIL 실패 (Code: $exit_code)"
            fi

            unset_pid "$key"
            running_jobs=$((running_jobs - 1))
            draw_status
        else
            NEW_ACTIVE_KEYS+=("$key")
        fi
    done
    ACTIVE_KEYS=("${NEW_ACTIVE_KEYS[@]}")

    sleep 0.5
done

# 6. 마무리
tput cnorm
echo ""
echo "🎉 모든 리뷰가 완료되었습니다!"
echo "📄 결과 확인: $OUTPUT_DIR"
echo ""
echo "📊 요약:"
for role in "${ROLES[@]}"; do
    echo "  - $role: $(get_status "$role")"
done
