...
/// If the line (trimmed) is an ID marker for a block, return that block id.
/// NOTE: (removed) This helper was unused; keep ID parsing localized to specific patch routines.
...
pub fn sync_page_to_markdown_after_block_change(
    ...
    if let Some(block_id) = changed_block_id {
        ...
        // If it doesn't exist (e.g. deletion), content patch will fail and we will fall back.
