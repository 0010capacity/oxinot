...
function createBlockRefClickHandler(): Extension {
  const handleClick = (event: MouseEvent, view: EditorView) => {
    ...
    // Ensure we have an Element (handle Text nodes by getting parent)
    const element = target instanceof Element ? target : null;
    if (!element) return false;

    const el = element.closest(
...
function createWikiLinkClickHandler(
  ...
  const handleClick = (event: MouseEvent, view: EditorView) => {
    ...
    const el = element.closest(".cm-wiki-link") as HTMLElement | null;
    if (!el) return false;
    event.preventDefault();
    event.stopPropagation();
...
function createExternalLinkClickHandler(): Extension {
  const handleClick = (event: MouseEvent, view: EditorView) => {
    ...
    const el = element.closest(".cm-link-text") as HTMLElement | null;
    if (!el) return false;
...
function createMouseDownHandler(): Extension {
  return EditorView.domEventHandlers({
    mousedown: (event, view) => {
      ...
      // Check if clicking on a clickable element (links, block refs)
      // If so, prevent default and let the click handler deal with navigation
      const target = event.target;
      if (target) {
        // Ensure we have an Element (handle Text nodes by getting parent)
        const element = target instanceof Element ? target : null;
        if (element) {
          ...
          if (clickableElement) {
            // Prevent cursor placement and focus change
